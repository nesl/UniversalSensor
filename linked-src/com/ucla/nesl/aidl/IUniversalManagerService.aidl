...
interface IUniversalManagerService {
	...
	boolean registerListener(IUniversalSensorManager mManager, String devID, int sType, int rateUs, float interval);
	boolean unregisterListener(String devID, int sType);
	boolean registerDriver(in Device device, IUniversalDriverManager mDriver, int sType, int rate);
	...
	oneway void onSensorChangedArray(in SensorParcel[] event, int length);
	String getDevID();
}